...
///
/// **In-lock commit:** the last successful intent recheck while holding `lifecycle_lock` is the
/// commit point; superseded applies roll back durable and live snapshots before returning.
///
/// **Post-lock fence:** after the lock drops, workers start only when intent, generation,
/// session, and lease still allow; a stale apply returns `Ok(())` and leaves the winner.
